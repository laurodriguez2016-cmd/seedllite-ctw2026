...
LAT0, LON0, HA, RADIO, PASO = ZONAS[sys.argv[1]]
...
for dla in ks:
    for dlo in ks:
        ...
        viejo = [v for m, v in s if m < "2022-01"]
        nuevo = [v for m, v in s if m >= "2023-01"]
        c_viejo = ciclos(viejo)
        amp_viejo = amplitud(viejo)
        amp_nuevo = amplitud(nuevo)
        med_nuevo = mediana(nuevo)
        # Diente de sierra REAL que se apaga: amplitud grande antes, muerta despues.
        if medidos >= 70 and c_viejo >= 6 and amp_viejo >= 0.35 and amp_nuevo <= 0.15:
...
json.dump([{"lat": h[0], "lon": h[1], "medidos": h[2], "ciclos_viejos": h[3],
          ...
          open(os.path.join(DIR, "abandono2_%s.json" % sys.argv[1]), "w"), indent=1)
